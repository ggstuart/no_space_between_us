...
export class Experiment {
    constructor(canvas, panel, nThings) {
        this.panel = panel;
        this.w = canvas.width;
        ...
        this.things = Array(nThings).fill().map(() => {
            return new One(
                ...
                0.1 + Math.random() * Math.random() * 0.5
            )
        });
    ...
    update() {
        for(const thing of this.things) {
            thing.update(this.elapsed);
            thing.loc.x = this.wrapX(thing.loc.x);
            thing.loc.y = this.wrapY(thing.loc.y);
        }
    }
}